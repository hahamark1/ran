...
parser.add_argument('--data1', type=str,  default='shit',
                    help='path to save the final model')
...
def add_to_pickle(data):
    if Path(args.data1 + '.pickle').is_file():
        save_file = pickle.load(open(args.data1 + '.pickle', 'rb'))
    else:
        save_file = []
    save_file.append(data)
    pickle.dump(save_file, open(args.data1 + '.pickle', 'wb'))
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            total_loss = 0
            start_time = time.time()
    add_to_pickle(('train', epoch, tot_loss[0]/train_data.size(0), math.exp(tot_loss[0]/train_data.size(0))))
...
try:
    for epoch in range(1, args.epochs+1):
        ...
        print('-' * 89)
        add_to_pickle(('valid', epoch, val_loss/val_data.size(0), math.exp(val_loss/val_data.size(0))))
